  ...
  try {
    ...
    if (!text || text.length < 40) {
      return NextResponse.json(
        {
          error:
            "Paste an abstract or upload a PDF with extractable text (at least ~40 characters).",
        },
        { status: 400 }
      );
    }

    const critique = await analyze({
      title,
      text,
      researchQuestion,
    });
    ...
    const status = err instanceof Error && err.name === "RateLimitError" ? 429 : 500;
